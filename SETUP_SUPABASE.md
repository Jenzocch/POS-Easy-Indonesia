...
除了上面的 App 內 UI，`src/utils/supabaseClient.js` 也支援用環境變數帶入設定，適合自動化建置 / 固定雲端環境的部署場景（例如 CI 直接打包一份已綁定特定 Supabase 專案的版本）：

1. 複製專案根目錄的 `.env.example` 為 `.env`
2. 填入 `VITE_SUPABASE_URL` 與 `VITE_SUPABASE_ANON_KEY`
3. 重新 `npm run dev` / `npm run build`

App 內 UI 設定（存在 localStorage）優先權高於環境變數；一般使用者照第 1–5 步用 UI 設定即可，不需要碰 `.env`。
