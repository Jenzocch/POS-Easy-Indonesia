...
function registerIpcHandlers() {
  ...
  ipcMain.handle('db:getKasbonRecords', (_e, memberId) => kasbon.listKasbonRecords(db, { memberId, limit: 10000 }))
  ipcMain.handle('db:getKasbonRecord', (_e, id) => kasbon.getKasbonRecordDetail(db, id))
  ipcMain.handle('db:addKasbonRecord', (_e, data) => kasbon.createKasbon(db, kasbon.getSubscription(db), data))
  ipcMain.handle('db:recordKasbonPayment', (_e, data) => kasbon.recordPayment(db, data))
  ipcMain.handle('db:getMemberKasbonBalance', (_e, memberId) => kasbon.getMemberKasbonSummary(db, memberId))
  ipcMain.handle('db:getKasbonPayments', (_e, recordId) => kasbon.listPayments(db, recordId))
  ipcMain.handle('db:getKasbonStoreTotal', () => kasbon.getStoreTotal(db))
  ipcMain.handle('db:getKasbonAgingReport', () => kasbon.getAgingReport(db))
