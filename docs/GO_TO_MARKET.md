...
| 功能（已存在） | 為什麼是訂閱利基點 |
|---|---|
| 雲端備份／同步（`src/utils/supabaseClient.js`）（⚠️ 現況為手動一鍵推／拉＋需自填金鑰，自動同步為開發項目） | 老闆最怕手機摔壞資料全沒（銷售、庫存、客人欠帳）。安心感是最強續訂理由；斷訂不毀生意（本地照常用），道德上站得住、客訴少 |
| QR 掃碼點餐（`public/menu/` + 即時訂單）（⚠️ 目前僅桌面 Electron 版可用） | 必須有伺服器在線上跑 → 天然月費、物理上不可破解 |
| 進階報表（ABC 分析、滯銷品、員工排行、損益；`src/utils/analytics.js`） | 開發成本已沉沒、感知價值高，且用的是客戶自己的數據 = 最好的推銷員 |
| 會員／積分（`MembersPage`）＋ Kasbon 賒帳（⚠️ **尚未實作**） | 印尼 warung 賒帳文化是剛需，數位化 kasbon 是殺手級功能——但 2026-07-07 代碼查證：現有代碼只有積分／儲值，Kasbon 需開發（任務 A8），**銷售前勿承諾此功能** |
...
| A8 | 產品印尼化補完＋Kasbon 開發：Kasbon 賒帳模組（會員掛帳／還款／欠款總覽）、雲端備份免填金鑰＋自動化、促銷編輯窗 NT$→Rp 與硬編中文 i18n 殘留清理、手機驗證改印尼 08xx 格式（`security.js`）、種子資料印尼化（供應商／促銷／會員等級門檻改 Rp 合理值） | 讓銷售話術與產品現況一致 | 無，開賣前優先 |

> ⚠️ **2026-07-07 代碼查證備註**：§1.2 表中 Kasbon 尚未實作；QR 點餐與訂單接收頁僅桌面 Electron 版可用；雲端備份為手動推拉；退貨入口目前僅在會員頁消費紀錄；電子支付僅為記帳分類（無 QRIS 金流整合，錢直接進老闆自己的 QRIS）。對外銷售一律以《docs/business/03_產品功能銷售手冊》為準（該手冊已逐功能按代碼實況撰寫），開發補完清單見任務 A8。
